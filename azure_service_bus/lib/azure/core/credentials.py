...
AccessToken.expires_on.__doc__ = (
...
class AzureKeyCredential:
    ...
    def update(self, key: str) -> None:
        ...
        if not key:
            raise ValueError(
                "The key used for updating can not be None or empty"
            )
        if not isinstance(key, str):
...
class AzureSasCredential:
    ...
    def update(self, signature: str) -> None:
        ...
        if not signature:
            raise ValueError(
                "The signature used for updating can not be None or empty"
            )
        if not isinstance(signature, str):
            raise TypeError(
                "The signature used for updating must be a string."
            )
        self._signature = signature
