...
class AbstractSpan(Protocol, Generic[SpanType]):
    ...
    def span(
        self, name: str = "child_span", **kwargs: Any
    ...
    @classmethod
    def link(
        cls, traceparent: str, attributes: Optional[Attributes] = None
    ) -> None:
        """
...
class HttpSpanMixin:
    ...
    def set_http_attributes(
        ...
        if parsed_url.hostname:
            self.add_attribute(
                HttpSpanMixin._NET_PEER_NAME, parsed_url.hostname
            )
        ...
        if response and response.status_code:
            self.add_attribute(
                HttpSpanMixin._HTTP_STATUS_CODE, response.status_code
            )
        else:
