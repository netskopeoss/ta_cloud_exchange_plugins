...
class AMQPClientAsync(AMQPClientSync):
        ...
        while retry_active:
            try:
                ...
                return await operation(
                    *args, timeout=absolute_timeout, **kwargs
                )
    ...
    async def auth_complete_async(self):
        ...
        if (
            ...
            and not await self._cbs_authenticator.handle_token()
        ):
...
class SendClientAsync(SendClientSync, AMQPClientAsync):
    ...
    async def _transfer_message_async(self, message_delivery, timeout=0):
        ...
        on_send_complete = partial(
            self._on_send_complete_async, message_delivery
        )
...
class ReceiveClientAsync(ReceiveClientSync, AMQPClientAsync):
    ...
    async def receive_messages_iter_async(
        self, timeout=None, on_message_received=None
    ):
