...
class AMQPClient(object):  # pylint: disable=too-many-instance-attributes
    ...
    def _close_link(self):
        if (
            self._link and not self._link._is_closed
        ...
        while retry_active:
            try:
                ...
                if not self._retry_policy.is_retryable(exc):
                    raise
                if absolute_timeout >= 0:
                    retry_active = self._retry_policy.increment(
                        retry_settings, exc
                    )
                    if not retry_active:
                        break
                    time.sleep(
                        self._retry_policy.get_backoff_time(
                            retry_settings, exc
                        )
                    )
    ...
    def open(self, connection=None):
        ...
        if connection:
            ...
            self._connection = Connection(
                ...
                ssl_opts={
                    "ca_certs": self._connection_verify or certifi.where()
                },
    ...
    def close(self):
        ...
        if self._keep_alive_thread:
            try:
                ...
                logging.debug(
                    "Keep alive thread failed to join.", exc_info=True
                )
            self._keep_alive_thread = None
        self._network_trace_params["amqpConnection"] = ""
    ...
    def auth_complete(self):
        ...
        """
        if (
            self._cbs_authenticator
            and not self._cbs_authenticator.handle_token()
        ):
    ...
    def mgmt_request(self, message, **kwargs):
        ...
        with self._mgmt_link_lock:
            try:
                ...
                mgmt_link = ManagementOperation(
                    self._session, endpoint=node, **kwargs
                )
...
class SendClient(AMQPClient):
    ...
    def __init__(self, hostname, target, **kwargs):
        ...
        self._max_message_size = kwargs.pop(
            "max_message_size", MAX_FRAME_SIZE_BYTES
        )
    ...
    def _process_send_error(
        message_delivery, condition, description=None, info=None
    ):
        try:
    ...
    def _send_message_impl(self, message, **kwargs):
        ...
        while (
            running
            and message_delivery.state not in MESSAGE_DELIVERY_DONE_STATES
        ):
    ...
    def send_message(self, message, **kwargs):
        ...
        self._do_retryable_operation(
            self._send_message_impl, message=message, **kwargs
        )
...
class ReceiveClient(AMQPClient):  # pylint:disable=too-many-instance-attributes
    ...
    def receive_message_batch(self, **kwargs):
        ...
        return self._do_retryable_operation(
            self._receive_message_batch_impl, **kwargs
        )
    ...
    def _message_generator(self, timeout=None):
        ...
        try:
            while receiving and not self._timeout_reached:
                if self._timeout > 0:
                    if (
                        time.time() - self._last_activity_timestamp
                        >= self._timeout
                    ):
