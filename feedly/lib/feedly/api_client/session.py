...
from ..api_client.data import FeedlyUser
from ..api_client.protocol import (
    APIClient,
    BadRequestAPIError,
    RateLimitedAPIError,
    ServerAPIError,
...
class Auth:
    ...
    def __init__(
        self, client_id: str = "feedlydev", client_secret: str = "feedlydev"
    ):
...
class FileAuthStore(Auth):
    ...
    def __init__(
        self,
        token_dir: Path = Path.home() / ".config/cte_feedly",
...
class FeedlySession(APIClient):
    def __init__(
        ...
        user_id: str = None,
        client_name="cte_feedly.python.client",
        ssl_verification=True,
        proxy={},
    ):
        """
        :param auth: either the access token str to use when making requests or an Auth object to manage tokens. If none
         are passed, it is assumed that the token and refresh token are correcly setup in the `~/.config/cte_feedly`
         directory. You can run setup_auth.py in the examples to get setup.
        :param api_host: the cte_feedly api server host.
        ...
        )  # as to treat cte_feedly server and connection errors identically
    ...
    def __repr__(self):
        return f"<cte_feedly client user={self.user.id}>"
    ...
    def do_api_request(
        ...
        """
        makes a request to the cte_feedly cloud API (https://developers.feedly.com), and parse the response
            ...
            while True:
                ...
                if self.rate_limiter.rate_limited:
                    until = datetime.datetime.fromtimestamp(
                        self.rate_limiter.until
                    ).isoformat()
                    raise ValueError(
                        f"Too many requests. Client is rate limited until {until}"
                    )
                ...
                if not conn_error and resp.ok:
                    ...
                    logger.warning(
                        "Error for %s: %s",
                        relative_url,
                        conn_error if conn_error else resp.text,
                    )
                    time.sleep(
                        2 ** (tries - 1)
                    )  # 1 second, then 2, 4, 8, etc.
            ...
            if code == 400:
                ...
                if (
                    ...
                    try:
                        ...
                        token_data = self.do_api_request(
                            "/v3/auth/token", data=auth_data
                        )
                        ...
                        logger.info(
                            "error refreshing access token", exc_info=e2
                        )
