...
from . import telemetry as msal_telemetry
...
class ClientApplication(object):
    ...
    def _build_telemetry_context(
        ...
        return msal_telemetry._TelemetryContext(
    ...
    def acquire_token_silent_with_error(
        ...
        assert isinstance(scopes, list), "Invalid parameter type"
        self._validate_ssh_cert_input_data(kwargs.get("data", {}))
        correlation_id = msal_telemetry._get_new_correlation_id()
        if authority:
    ...
    def _acquire_token_silent_from_cache_and_possibly_refresh_it(
        ...
        if not (force_refresh or claims_challenge):  # Bypass AT when desired or using claims
            ...
            matches = self.token_cache.find(
                ...
                query=query)
            now = time.time()
            refresh_reason = msal_telemetry.AT_ABSENT
            for entry in matches:
                expires_in = int(entry["expires_on"]) - now
                if expires_in < 5*60:  # Then consider it expired
                    refresh_reason = msal_telemetry.AT_EXPIRED
                ...
                if "refresh_on" in entry and int(entry["refresh_on"]) < now:  # aging
                    refresh_reason = msal_telemetry.AT_AGING
                    break  # With a fallback in hand, we break here to go refresh
                self._build_telemetry_context(-1).hit_an_access_token()
                return access_token_from_cache  # It is still good as new
        else:
            refresh_reason = msal_telemetry.FORCE_REFRESH  # TODO: It could also mean claims_challenge
    ...
    def acquire_token_by_refresh_token(self, refresh_token, scopes, **kwargs):
        ...
        telemetry_context = self._build_telemetry_context(
            ...
            refresh_reason=msal_telemetry.FORCE_REFRESH)
    ...
    def acquire_token_by_username_password(
        ...
        if not self.authority.is_adfs:
            user_realm_result = self.authority.user_realm_discovery(
                username, correlation_id=headers[msal_telemetry.CLIENT_REQUEST_ID])
...
class PublicClientApplication(ClientApplication):  # browser app or mobile app
    ...
    def initiate_device_flow(self, scopes=None, **kwargs):
        ...
        flow = self.client.initiate_device_flow(
            ...
            headers={msal_telemetry.CLIENT_REQUEST_ID: correlation_id},
            **kwargs)
