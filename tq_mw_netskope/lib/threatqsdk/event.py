...
class Event(ThreatQuotientObject):
    ...
    def fill_from_api_response(self, api_response):
        ...
        if 'title' in api_response:
            ...
            self.title = ''
        self.desc = api_response['description']
    ...
    def upload(self, sources=None):
        """ Upload this event to ThreatQuotient.
        Always creates a new event

        :raises: :py:class:`~threatqsdk.exceptions.UploadFailedError` if
                the API does not give us an event ID back.
        :raises: :py:class:`~exceptions.ValueError` if the ``typename`` or
                ``happened_at`` properties have not been set

        :param sources: Converted to a list of sources as defined by
                :py:func:`~threatqsdk.source.make_source_list`

        :returns: the event ID produced
        """
        data = self._to_dict()
        ...
        if self.typename == "Spearphish":
            ...
            data = {"title": self.title,
                    "happened_at": self.happened_at, "description": None}
            result = self.tq.put(
