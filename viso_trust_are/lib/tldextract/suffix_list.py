...
PUBLIC_SUFFIX_RE = re.compile(
    r"^(?P<suffix>[.*!]*\w[\S]*)", re.UNICODE | re.MULTILINE
)
    ...
    try:
        for url in urls:
            try:
                ...
                LOG.exception(
                    "Exception reading Public Suffix List url %s", url
                )
    finally:
...
def extract_tlds_from_suffix_list(
    suffix_list_text: str,
    ...
    public_tlds = [
        m.group("suffix") for m in PUBLIC_SUFFIX_RE.finditer(public_text)
    ]
    private_tlds = [
        m.group("suffix") for m in PUBLIC_SUFFIX_RE.finditer(private_text)
    ]
...
def _get_suffix_lists(
    ...
    try:
        ...
        if fallback_to_snapshot:
            maybe_pkg_data = pkgutil.get_data(
